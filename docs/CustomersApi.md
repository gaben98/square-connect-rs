...
**sort_field** | Option<[**CustomerSortField**](CustomerSortField.md)> | Indicates how customers should be sorted.  The default value is `DEFAULT`. |  |
**sort_order** | Option<[**SortOrder**](SortOrder.md)> | Indicates whether customers should be sorted in ascending (`ASC`) or descending (`DESC`) order.  The default value is `ASC`. |  |
